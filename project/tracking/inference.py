...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variables=['Ghost0','Ghost1','Pacman','Observation0','Observation1']
    edges=[('Pacman','Observation0'),('Pacman','Observation1'),('Ghost0','Observation0'),('Ghost1','Observation1')]
    variableDomainsDict['Pacman']=[(x,y) for x in range(X_RANGE) for y in range(Y_RANGE) ]
    ...
    variableDomainsDict['Observation0']=[i for i in range(manhattanDistance((0,0),(X_RANGE-1,Y_RANGE-1))+MAX_NOISE+1)]
    variableDomainsDict['Observation1']=[i for i in range(manhattanDistance((0,0),(X_RANGE-1,Y_RANGE-1))+MAX_NOISE+1)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for joinVariable in eliminationOrder:
            if joinVariable in eliminationVariables:
                currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total_value = self.total()
        if total_value == 0:
            raise Exception("Total value of the distribution is 0, cannot sample.")
        self.normalize()
        rand = random.random()
        cumulative = 0.0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            prob = self.getObservationProb(observation, pac_pos, pos, jail_pos)
            self.beliefs[pos] = self.beliefs[pos] * prob
            #P(Ghost在pos | observation) ∝ P(observation | Ghost在pos) × P(Ghost在pos)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_pos in self.allPositions:
            ...
            for new_pos in newPosDist.keys():
                new_beliefs[new_pos] += newPosDist[new_pos] * self.beliefs[old_pos]
        self.beliefs = new_beliefs
        self.beliefs.normalize()
        #P(Ghost在new_pos_t+1) = Σ P(Ghost在new_pos_t+1 | Ghost在old_pos_t) × P(Ghost在old_pos_t)
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        num_legal_pos = len(self.legalPositions)
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % num_legal_pos])
