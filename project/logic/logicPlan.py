...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    return findModel(combined) == False
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in non_outer_wall_coords:
        pac_symbols.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(pac_symbols))
    for direction in DIRECTIONS:
        action_symbols.append(PropSymbolExpr(direction, time=t))
    pacphysics_sentences.append(exactlyOne(action_symbols))

    if sensorModel:
       pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    if successorAxioms and walls_grid is not None and t>0:
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            pac_pos.append(PropSymbolExpr(pacman_str, x, y, time=t))
            KB.append(pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid))
        KB.append(exactlyOne(pac_pos))
        ...
        for direction in actions:
            possible_actions.append(PropSymbolExpr(direction, time=t))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        model = findModel(conjoin(KB + [conjoin(goal_state)]))
        if model:
            return extractActionSequence(model, actions)
        ...
        for x, y in food:
            ...
            avoid_eat_food = ~pac_pos & food_t

            KB.append(avoid_eat_food >> next_food)
            KB.append(eat_food >> ~next_food)
    return []
    "*** END YOUR CODE HERE ***"


def AddpacphysicsAxioms(agent,KB,t,all_coords,non_outer_wall_coords,walls_grid):
    """
    Helper function to call pacphysicsAxioms and add the result to the KB.
    """
...
def FindPacPos(t,KB,agent,possible_loc,coord):
    pac_loc=PropSymbolExpr(pacman_str, coord[0], coord[1], time=t)
    KB_c=conjoin(KB)
    model= findModel(conjoin([KB_c,pac_loc]))
    if model:
        possible_loc.append(coord)
    elif (entails(KB_c, pac_loc)):
        KB.append(pac_loc)
    else:
        KB.append(~pac_loc)


def FindProvWall(t,KB,agent,possible_loc,coord,map):
    wall_loc=PropSymbolExpr(wall_str, coord[0], coord[1])
    KB_c=conjoin(KB)
    if (entails(KB_c, wall_loc)):
        KB.append(wall_loc)
        map[coord[0]][coord[1]]=1
    elif entails(KB_c, ~wall_loc):  # 一定不是墙
        KB.append(~wall_loc)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        possible_locations = []
        AddpacphysicsAxioms(agent,KB,t,all_coords,non_outer_wall_coords,walls_grid)
        for coord in non_outer_wall_coords:
            FindPacPos(t,KB,agent,possible_locations,coord)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        AddpacphysicsAxioms(agent,KB,t,all_coords,non_outer_wall_coords,known_map)
        for coord in non_outer_wall_coords:
            FindProvWall(t,KB,agent,[],coord,known_map)
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield known_map


def AddpacphysicsAxioms2(agent,KB,t,all_coords,non_outer_wall_coords,walls_grid):
    """
    Helper function to call pacphysicsAxioms and add the result to the KB.
    """
...
def FindPacPos2(t,KB,agent,possible_loc,coord):
    pac_loc=PropSymbolExpr(pacman_str, coord[0], coord[1], time=t)
    KB_c=conjoin(KB)
    model= findModel(conjoin([KB_c,pac_loc]))
    if model:
...
def FindProvWall2(t,KB,agent,possible_loc,coord,map):
    wall_loc=PropSymbolExpr(wall_str, coord[0], coord[1])
    KB_c=conjoin(KB)
    if (entails(KB_c, wall_loc)):
        KB.append(wall_loc)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        "*** END YOUR CODE HERE ***"
        possible_locations = []
        AddpacphysicsAxioms2(agent,KB,t,all_coords,non_outer_wall_coords,known_map)
        for coord in non_outer_wall_coords:
            FindPacPos2(t,KB,agent,possible_locations,coord)
            FindProvWall2(t,KB,agent,[],coord,known_map)
        agent.moveToNextState(agent.actions[t])
