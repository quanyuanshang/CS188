...
class Grades:
  ...
  def grade(self, gradingModule, exceptionMap = {}, bonusPic = False):
    ...
    """
    if self.gsOutput:
      late_gradescope = self.isGradeScopeLate()
...
run 'python submit.py' and submit 'search.zip' to Gradescope to receive
...
""")
    ...
    if self.edxOutput:
        self.produceOutput()
    if self.gsOutput:
        self.produceGradeScopeOutput(late_gradescope)
  ...
  def isGradeScopeLate(self):
    from pathlib import Path
    from datetime import datetime
    path = Path('submission_metadata.json')
    if not path.is_file():
      raise Exception("Gradescope: submission_metadata.json not found")
    
    with open(path, 'r') as f:
      metadata = json.load(f)
    create_time = datetime.fromisoformat(metadata["created_at"])
    due_date = datetime.fromisoformat(metadata["assignment"]["due_date"])
    return create_time > due_date

  def produceGradeScopeOutput(self, late_gradescope):
    out_dct = {}
    ...
    out_dct['output'] = '\n'.join([
        ...
        'Starting on %d-%d at %d:%02d:%02d' % self.start,
        'Finished at %d:%02d:%02d' % time.localtime()[3:6],
        "Total score (%d / %d)" % (total_score, total_possible)
    ])
    if late_gradescope:
      out_dct['output'] += "\n\nWARNING: This submission is late according to Gradescope's timestamp. You will receive a 50% penalty on your total score."
      out_dct['score'] = total_score * 0.5
    ...
    with open('gradescope_response.json', 'w') as outfile:
        json.dump(out_dct, outfile, ensure_ascii=False)
    return
