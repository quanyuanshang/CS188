...
def sentence1() -> Expr:
    ...
    exp1=Expr('A') | Expr('B')
    exp2=~Expr('A') % (~Expr('B') | Expr('C'))
    exp3=~Expr('A') | ~Expr('B') | Expr('C')
    return conjoin([exp1,exp2,exp3])
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    expr1=pacman_alive_1 % (pacman_alive_0&~pacman_killed_0 | (~pacman_alive_0&pacman_born_0))
    expr2=~(pacman_alive_0 & pacman_born_0)
    expr3=pacman_born_0
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    expr= ~conclusion & premise
    return findModel(expr)==False
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([~l1 | ~l2 for l1, l2 in itertools.combinations(literals, 2)])
    "*** END YOUR CODE HERE ***"
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacman_pos_0=PropSymbolExpr(pacman_str,x0,y0,time=0)
    pacman_pos_1=PropSymbolExpr(pacman_str,x1,y1,time=1)
    action_0=PropSymbolExpr(action0,time=0)
    action_1=PropSymbolExpr(action1,time=1)

    KB.append(pacman_pos_0)
    KB.append(action_0)
    KB.append(action_1)
    ...
    model1=findModel(conjoin(KB+[pacman_pos_1]))
    model2=findModel(conjoin(KB+[~pacman_pos_1]))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        succ_axioms = [pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid) for x, y in non_wall_coords]
        succ_axioms = [ax for ax in succ_axioms if ax is not None]
        if succ_axioms:
...
def foodLogicPlan(problem) -> List:
    ...
    KB.append(PropSymbolExpr(pacman_str,x0,y0,time=0))
    food_pos=[PropSymbolExpr(food_str,x,y,time=0) for x,y in food]
    KB.append(conjoin(food_pos))
    for t in range(50):
        print(f"t={t}")
        KB.append(exactlyOne([PropSymbolExpr(pacman_str,x,y,time=t) for x,y in non_wall_coords]))
        
        goal_state=[~PropSymbolExpr(food_str,x,y,time=t) for x,y in food]
        goal_state=conjoin(goal_state)
        model=findModel(conjoin(KB+[goal_state]))
        if model:
            return extractActionSequence(model,actions)
        KB.append(exactlyOne([PropSymbolExpr(direction,time=t) for direction in actions]))
        succ_axioms = [pacmanSuccessorAxiomSingle(x, y, t+1, walls) for x, y in non_wall_coords]
        succ_axioms = [ax for ax in succ_axioms if ax is not None]
        if succ_axioms:
           KB.append(conjoin(succ_axioms))
        for x,y in food:
            ...
            next_food_pos=PropSymbolExpr(food_str,x,y,time=t+1)
            eat_food=pac_pos&food_t
            avioid_eat=~pac_pos&food_t
            KB.append(avioid_eat>>next_food_pos)
            KB.append(eat_food>>~next_food_pos)
...
#______________________________________________________________________________
# QUESTION 6
def addpacphysics1(KB,agent,t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None) :
...
def addpacphysics2(KB,agent,t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None) :
...
def finfpossiblelocations(KB,t: int, coord,possible_locations):
    
    pac_pos=PropSymbolExpr(pacman_str,coord[0],coord[1],time=t) 
    KB_c=conjoin(KB)
    model=findModel(conjoin(KB+[pac_pos]))
    if model:
        possible_locations.append(coord)
    elif entails(KB_c,pac_pos):
        KB.append(pac_pos)
    else:
        KB.append(~pac_pos)

def findprovablewall(KB, coord,known_map):
    wall_pos=PropSymbolExpr(wall_str,coord[0],coord[1])
    
    
    if entails(conjoin(KB),wall_pos):
        known_map[coord[0]][coord[1]]=1
        KB.append(wall_pos)
    elif entails(conjoin(KB),~wall_pos):
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        "*** END YOUR CODE HERE ***"
        possible_locations=[]
        addpacphysics1(KB,agent,t,all_coords,non_outer_wall_coords,walls_grid)
        for coord in non_outer_wall_coords:
            finfpossiblelocations(KB,t,coord,possible_locations)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        "*** END YOUR CODE HERE ***"
        addpacphysics1(KB,agent,t,all_coords,non_outer_wall_coords,known_map)
        for coord in non_outer_wall_coords:
            findprovablewall(KB,coord,known_map)
        agent.moveToNextState(agent.actions[t])
        yield known_map
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        "*** END YOUR CODE HERE ***"
        possible_locations=[]
        addpacphysics2(KB,agent,t,all_coords,non_outer_wall_coords,known_map)
        for coord in non_outer_wall_coords:
            findprovablewall(KB,coord,known_map)
            finfpossiblelocations(KB,t,coord,possible_locations)
        agent.moveToNextState(agent.actions[t])
